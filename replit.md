...
-   **CaskSense Labs (`/labs/*`)**: A parallel experimental UX layer focused on refined whisky tasting. Dedicated shell (`LabsLayout.tsx`) with warm dark/light premium theme (`labs-theme.css`, CSS variables with `.labs-light` override), Apple-inspired design, bottom nav (Tastings/Explore/Discover/Taste/Circle). M2↔Labs switching via profile menu (available to all users). Auto-resumes session on load via `tryAutoResume()`. Reuses all existing backend APIs, auth, and database. Files: `client/src/labs/` directory.
