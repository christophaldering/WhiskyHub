...
-   **Navigation Structure (Simple Mode)**: 5-tab bottom nav: Join (`/enter`), Log (`/log-simple`), Host (`/host`), My Taste (`/my-taste`), Discover (`/analyze`). Landing page shows 5 matching CTA buttons. Simple Mode pages wrapped in `SimpleShell`. Host and Discover are hub pages linking to legacy features.
