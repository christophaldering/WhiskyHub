...
-   **Downloads / Printable Sheets**: PDF tasting sheets use strict 2-page format: Page 1 is a cover page (tasting title, cover image, host name, date, location, code, participant QR + whisky lineup for open / numbered sample grid for blind). Page 2 is a compact scoring sheet with ALL whiskies on one page, each with 5 individually scored criteria (Nose, Palate, Finish, Balance, Overall) as numbered 1-10 circles. Layout dynamically scales (circle size, font, spacing) based on whisky count (supports 3-12+ whiskies). Batch PDF generates 2 pages per participant with shared cover image for performance. Key functions: `drawCoverPage()`, `drawScoringPage()`, `generateTastingNotesSheet()`, `generateBlindEvaluationSheet()`, `generateBatchPersonalizedPdf()` in `client/src/components/printable-tasting-sheets.tsx`.
