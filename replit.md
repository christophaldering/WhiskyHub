...
-   **Naked-first Progressive Reveal**: Non-host participants arriving at `/tasting/:id` see a simplified "Naked-first" rating view (NakedFirstView) by default when the session is open. This minimal view focuses on rating with an info drawer (ProgressiveRevealDrawer) for context, comparison, and tools — gated by blind mode and experience level. Users can expand to the full tasting room. The `/tasting/:id` route renders outside the main Layout, wrapped in a minimal TastingShell header. Blind state logic is centralized in `useBlindState` hook (`client/src/hooks/use-blind-state.ts`), replacing 3 duplicate implementations.
