...
- **Dual UI Variant System (Feb 2026)**: Two complete UI shells — "Classic" (original feature-module navigation at `/app/*`) and "Lounge" (room-based navigation at `/lounge/*` with warm espresso/amber/sand styling). Both variants share the SAME database, API, and business logic — only the navigation structure, layout, and visual emphasis differ. Lounge organizes features into "rooms": Lounge Home (curated welcome + radar + highlights), Tastings, Guests (friends/activity/related palates), My Salon (profile/journal/whiskies/badges + combined "My Taste" tab page), and Library (recommendations/comparison/encyclopedia). Route mapping layer (`client/src/lib/route-mapping.ts`) enables instant bidirectional switching with context preservation. Theme auto-syncs based on URL path. Chart colors theme-aware via `useChartColors()` and `useFlavorWheelColors()` hooks. Full EN/DE translations for all lounge labels.
