...
-   **Experience Levels**: Four progressive levels — guest (Just Tasting), explorer (Entdecker), connoisseur (Kenner), analyst. Levels are UI presets only (default panel expansion, copy density, guidance text) — NOT feature gating. All navigation items, pages, and features are accessible at every level. Only role-based restrictions remain (admin panel, whisky database access). The `isSimplified` flag (guest/explorer) controls rating UI density (3 vs 4 dimensions) as a presentation choice, not a capability restriction. Landing page features Just Tasting as a full-width hero card emphasizing minimal tech, with Explorer/Connoisseur/Analyst below.
-   **Navigation Structure**: 4-section sidebar (Tastings, Tagebuch, Entdecken, Konto). All sections visible to all experience levels (no level-based gating). Tastings contains session management (Sessions, Lobby, Calendar, Host Dashboard, Recap, My Tastings). Tagebuch has personal whisky items (Journal, Tasted, Collection, Wishlist). Entdecken has 3 collapsible subgroups: Community (Rankings, Activity, Taste Twins, Leaderboard, Friends), Analytics & Tools (Comparison, Recommendations, Pairings, Templates, Analytics, Benchmark), Wissen & Forschung (Lexicon, Research, Distilleries incl. Map, Bottlers, Whisky Database). Konto merges profile and account (Profile, Whisky Profile, Account, Data Export, Help, About, Features, Donate, Admin Panel). Only role-based restrictions remain: Whisky Database requires host/admin/canAccessWhiskyDb, Admin Panel requires admin role. Accordion behavior: only one top-level section expanded at a time. Entdecken subgroups have independent collapse state persisted in localStorage. News/notifications shown as a section on Home page instead of separate nav item. Badges hidden from menu and Home page but route preserved.
