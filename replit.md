...
-   **My Taste (Personal Dashboard)**: Personal whisky profile hub requiring email+password sign-in (no naked participants). Shows Taste Snapshot (stability/exploration/smoke scores), AI Taste Insight, and 4 grouped sections: Mein Profil (Flavor Profile — single scrollable page with aroma wheel, taste structure, region/cask/peat preferences, rating style, top whiskies), Auswertungen (Analytics `/my-taste/analytics` threshold-gated at 10, Comparison `/my-taste/compare`, Benchmark Analyzer `/my-taste/benchmark`, Empfehlungen `/my-taste/recommendations`), Meine Daten (Journal `/my-taste/journal` with 3 tabs: Journal/Verkostet/Export, plus Tasting Recap link → `/sessions`), Meine Sammlung (Collection/Whiskybase sync, Wishlist).
-   **Discover (External World Hub)**: Separated from personal features. Sections: Sozial (Taste Twins, Whisky Friends, Rankings, Leaderboard, Activity Feed), Empfehlungen (cross-link to `/my-taste/recommendations`), Wissen (Lexicon at `/discover/lexicon`, Distilleries at `/discover/distilleries` with integrated List/Map toggle, Tasting Guide at `/guide`, Whisky Database). Live platform stats (taster count, whisky count) shown at top.
-   **Native Dark Warm Detail Pages**: `/my-taste/flavors` (redirects to `/my-taste/profile`), `/my-taste/compare` (RadarChart side-by-side whisky comparison — compares your personal ratings), `/my-taste/analytics` (2 cards: Taste Evolution, Rating Consistency), `/my-taste/benchmark` (standalone Benchmark Analyzer with SimpleShell), `/my-taste/recommendations` (personalized whisky suggestions, moved from Discover), `/my-taste/profile` (unified Flavor Profile: aroma wheel + radar chart + region/cask/peat breakdowns + rating style + top whiskies — single scrollable page), `/my-taste/journal` (3 tabs: Journal, Verkostet, Export), `/my-taste/collection` (Whiskybase collection with sync), `/my-taste/wishlist` (wishlist with AI identification), `/discover/lexicon` (searchable accordion with i18n EN/DE), `/discover/community` (tabbed Twins/Rankings/Leaderboard), `/discover/distilleries` (searchable/filterable encyclopedia with List/Map toggle), `/discover/recommendations` (redirects to `/my-taste/recommendations`), `/discover/database` (searchable whisky database), `/discover/activity` (activity feed timeline), `/guide` and `/discover/guide` (illustrated tasting assessment guide, dark warm), `/recap/:id` (tasting recap with PDF export).
