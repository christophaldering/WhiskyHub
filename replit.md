...
-   **Simple Mode (`/enter`, `/log-simple`, `/my-taste`)**: Minimal UI for first-time users with client-side rate limiting. `/log-simple` IdentifyPicker offers two prominent Apple-style cards (Photo + File Upload) above a divider, then scan/gallery/type as secondary options. File upload accepts Excel/CSV/PDF/DOCX, calls `/api/tastings/ai-import`, and auto-fills whisky fields from extracted data. i18n keys under `logSimple.*` namespace.
