...
-   **Navigation Structure**: 6-section sidebar (Genuss, Pro, Profil, Wissen, Über, Admin). Genuss contains core tasting items (Lobby, Sessions, Journal, Tasted, Collection, Wishlist, Recap). Pro has advanced tools (Comparison, Templates, Pairings, Benchmark, Whisky Database, Analytics, Data Export) — visible at connoisseur+ level. Profil covers personal pages and social (Profile, Whisky Profile, Recommendations, Taste Twins, Friends, Community Rankings, Activity, Leaderboard, Account) — visible at explorer+ level. Wissen has knowledge base (Lexicon, Distilleries incl. Map, Bottlers, Research) — visible at connoisseur+ level. Über has Help, About, Features, Donate, Landing Page — always visible. Admin has Admin Panel — admin role only. Accordion behavior: only one section expanded at a time. News/notifications shown as a section on Home page instead of separate nav item. Badges hidden from menu and Home page but route preserved. Calendar page still accessible via route but removed from nav menu (old `/reminders` redirects to `/sessions`). Whisky Database in Pro (analyst+ level, host/admin access).
