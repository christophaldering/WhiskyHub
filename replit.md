...
-   **Native Dark Warm Detail Pages**: `/my-taste/flavors` (redirects to `/my-taste/profile`), `/my-taste/compare` (RadarChart side-by-side whisky comparison — compares your personal ratings), `/my-taste/analytics` (2 cards: Taste Evolution, Rating Consistency), `/my-taste/profile` (unified Flavor Profile: aroma wheel + radar chart + region/cask/peat breakdowns + rating style + top whiskies — single scrollable page), `/my-taste/journal` (3 tabs: Journal, Verkostet, Export), `/my-taste/collection` (Whiskybase collection with sync), `/my-taste/wishlist` (wishlist with AI identification), `/discover/lexicon` (searchable accordion with i18n EN/DE), `/discover/community` (tabbed Twins/Rankings/Leaderboard), `/discover/distilleries` (searchable/filterable encyclopedia), `/discover/recommendations` (personalized whisky suggestions), `/discover/database` (searchable whisky database), `/discover/activity` (activity feed timeline), `/recap/:id` (tasting recap with PDF export).
