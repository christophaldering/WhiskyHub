...
-   **Navigation Structure**: 6-section sidebar (Genuss, Pro, Profil, Wissen, Über, Admin). Genuss contains core tasting items (Lobby, Sessions, Journal, Tasted, Collection, Wishlist, Recap). Pro has advanced tools (Comparison, Templates, Pairings, Library, Database, Analytics, Data Export). Profil covers personal pages and social (Profile, Whisky Profile, Recommendations, Taste Twins, Friends, Rankings, Activity, Leaderboard, Account). Wissen has knowledge base (Lexicon, Distilleries, Map, Bottlers, Research). Über has Help, About, Features, Donate. Admin visible only to admin role. Legacy reference of 8 groups: Dashboard (lobby, sessions), My Profile (profile, "Mein Whisky-Profil" w/ flavor wheel tab), My Tastings (history, "Gastgeber-Dashboard", recap — with export notes accessible from recap), My Whiskys (tasted, journal, collection, wishlist), Tools & Analytics (recommendations, comparison, templates, pairings, whisky library, whisky database, analytics), Community, Whisky Knowledge, About CaskSense, Account (settings, data export, admin). News/notifications shown as a section on Home page instead of separate nav item. Badges hidden from menu and Home page but route preserved. Calendar page still accessible via route but removed from nav menu (old `/reminders` redirects to `/sessions`). Whisky Database in Tools & Analytics (analyst+ level, host/admin access).
