...
- **Dual UI Theme System (Feb 2026)**: Two visual modes — "Classic" (original muted slate blue) and "Lounge" (warm espresso/amber/sand). Theme controlled via `data-ui-theme` attribute on root element, independent of dark/light mode. Toggle in sidebar footer, mobile header, and landing page nav. Lounge mode adds welcome hero on dashboard, increased spacing, deeper shadows. Chart colors (Recharts radar, flavor wheel, bar charts) are theme-aware via `useChartColors()` and `useFlavorWheelColors()` hooks in `client/src/lib/theme-colors.ts`. Persisted in localStorage via Zustand. Full EN/DE translations for theme labels.
