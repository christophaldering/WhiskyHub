...
-   **Score Normalization**: All scores are normalized to a 0–100 scale for cross-source comparison. Live ratings: `normalizedScore = overall × (100 / ratingScale)`. Historical entries: `normalizedTotal = totalScore × 10` (original 0–10 scale). Normalization is set at write-time (rating save, historical import) and backfilled on startup via `backfillNormalizedScores()`. Taste Twins uses `normalizedScore` for Pearson correlation. APIs return both raw and normalized scores (`avgScoreNormalized`, `sourceScale`).
