...
### Frontend (`client/`)
The frontend is a React application built with Vite, using Wouter for routing, TanStack React Query for server state management, and Zustand for client-side state. The UI is developed with shadcn/ui (new-york style) based on Radix UI and Tailwind CSS, featuring a custom muted slate blue theme and light mode. Framer Motion handles animations, Recharts provides data visualizations, and react-i18next is used for internationalization (English and German). Fonts are Playfair Display and Inter. PWA support is included for installability and offline access. UI elements adapt dynamically to chosen rating scales (5/10/20/100 points). All users get the full feature set regardless of experience level. A shared `PageLayout` component (`client/src/components/page-layout.tsx`) standardizes the inner page scaffold with consistent icon+title header, optional subtitle, optional primaryAction (top-right), and max-w-5xl container across all main pages.
...
-   **Shared Constants**: `shared/constants.ts` exports `ACTIVE_STATUSES = ["open", "closed", "reveal"]` used consistently across client (layout.tsx, sessions.tsx) and server (routes.ts) to avoid drift.
-   **Navigation Structure (v4 Tasting-Centric)**: Desktop sidebar has 5 sections with 12 links (excluding admin): Jetzt (/now → redirects to /tasting/sessions, **only visible when active sessions exist**), Tastings (Lobby /tasting, Sessions /tasting/sessions, Calendar /tasting/calendar), Journal (/my/journal, Collection /my/collection, Wishlist /my/wishlist), Entdecken (Discover /discover, Distilleries /discover/distilleries, Community /discover/community), Profil (Profile /profile, Help /profile/help), Admin (/admin, admin-only). Account (/profile/account) removed from sidebar, accessible via button on /profile page. "Active session" detection uses `ACTIVE_STATUSES` from shared constants. Mobile bottom nav: 5 tabs (Jetzt always visible, Sessions, Journal, Entdecken, Profil). All old routes preserved as redirect aliases (e.g. /comparison → /my/journal?tab=compare). Whisky Database (/discover/database) remains gated to host/admin/whitelisted but removed from sidebar. Host Dashboard (/tasting/host) is client-side gated: non-hosts with 0 hosted tastings see friendly redirect; admins always allowed.
-   **Sessions Page Architecture**: Single tab layer in `tasting-sessions.tsx` with 4 tabs (Sessions, Meine Tastings, Host Dashboard, Recap) controlled via URL `?tab=` query param. `sessions.tsx` is a headerless content component (join/create/session lists) rendered inside the Sessions tab. No duplicate title or nested containers.
