...
-   **Navigation Structure (v5 Mobile-First)**: Desktop sidebar has 4 sections: Tastings (Sessions /tasting/sessions, Vorlagen & Pairings /tasting, Kalender /tasting/calendar), Tagebuch (Private Verkostungen /my/journal, Collection /my/collection, Wishlist /my/wishlist), Entdecken (Discover /discover, Distilleries /discover/distilleries, Community /discover/community), Profil (Profile /profile, Help /profile/help), Admin (/admin, admin-only). "Jetzt" section removed from sidebar — /now route still exists as redirect to /tasting/sessions. Mobile: NO hamburger menu, NO sidebar drawer. Only BottomNav with 5 tabs: Home (/tasting, Lobby), Tastings (/tasting/sessions), Tagebuch (/my/journal), Entdecken (/discover), Profil (/profile). Profile page contains quick-links to Account, Help, About, Features, Donate, Admin (role-gated), plus Language/Theme toggles. Labels: "Kalender" (not "Tasting-Kalender"), "Private Verkostungen" (not "Mein privates Verkostungstagebuch"). All old routes preserved as redirect aliases.
