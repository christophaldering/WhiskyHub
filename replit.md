...
-   **My Taste (Personal Dashboard)**: Personal whisky profile hub requiring email+password sign-in, showing taste snapshot, AI insights, flavor profiles, analytics, recommendations, drams, and collection management. Sections: Mein Profil (Flavor Profile, Aroma Wheel `/my-taste/wheel`, Settings `/my-taste/settings`), Auswertungen (Analytics, Compare, Benchmark, Food Pairings `/my-taste/pairings`, Recommendations), Meine Drams (`/my-taste/drams` — "All Drams / Tastings / Export" tabs), Meine Sammlung (Collection, Wishlist). Restructured via `MY_TASTE_STRUCTURE` flag in `client/src/lib/config.ts`. V2 layout (`v2_experience_first`): Drams section primary (My Drams, Tasting Recap, Flavor Profile + primary CTA), Collection section secondary. Legacy layout restorable by setting flag to `"legacy"`. All subpages include a reusable `BackButton` component (`client/src/components/back-button.tsx`) for consistent navigation. Terminology standardized: "Journal" → "My Drams" / "Meine Drams" throughout. Old route `/my-taste/journal` redirects to `/my-taste/drams`.
