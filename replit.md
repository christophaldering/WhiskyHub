...
18. **Tasting Note Generator**: Collapsible helper below the notes field in the evaluation form. Presents curated whisky flavor/aroma categories (fruity, floral, sweet, spicy, woody, smoky, malty, etc.) for nose, palate, and finish. Users select flavors via clickable badges, then generate a structured natural-language tasting note. Supports insert (replace) and append modes. Full EN/DE bilingual flavor vocabulary and note generation.
19. **Reflection phase**: Optional post-tasting reflection enabled at session creation. Supports standard 4 prompts or custom host-defined prompts (1–5). Three visibility modes: named, anonymous, or optional (participant chooses). Uses `reflection_entries` table. Panel shows per-prompt submission with all reflections aggregated.
