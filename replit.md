...
-   **Personalization & Analytics**: Features participant profiles, a whisky journal, achievement badges, personal flavor profiles (radar charts), whisky recommendations, side-by-side comparisons, and a flavor wheel. Privacy-respecting per-tasting analytics are provided. Native Dark Warm analytics page at `/my-taste/analytics` with two cards: Taste Evolution (monthly average line chart), Rating Consistency (stability score + stats grid). Taste Profile and Taste Map content moved to the unified Flavor Profile page.
...
-   **Host Wizard (Simple Mode)**: A 4-step inline wizard for creating sessions, adding whiskies, inviting participants, and running live tastings. Host page includes a "Tasting erstellen" section with 3 creation method cards: Manual Entry (wizard), Photo/AI Identification (`/photo-tasting`), and Excel/CSV Import (dialog). Also links to AI Lineup Suggestions (`/ai-curation`).
...
-   **My Taste (Personal Dashboard)**: Personal whisky profile hub requiring email+password sign-in (no naked participants). Shows Taste Snapshot (stability/exploration/smoke scores), AI Taste Insight, and 4 grouped sections: Mein Profil (Flavor Profile, Settings & Profile `/my-taste/settings`, Aroma Wheel `/my-taste/wheel`), Auswertungen (Analytics `/my-taste/analytics` threshold-gated at 10, Comparison `/my-taste/compare`, Empfehlungen `/my-taste/recommendations`, Benchmark Analyzer `/my-taste/benchmark`, Food Pairings `/my-taste/pairings`), Meine Daten (Journal `/my-taste/journal` with 3 tabs: Journal/Verkostet/Export, plus Tasting Recap link → `/sessions`), Meine Sammlung (Collection/Whiskybase sync, Wishlist).
-   **Discover (External World Hub)**: Separated from personal features. Sections: Sozial (Taste Twins, Whisky Friends, Rankings, Leaderboard, Activity Feed, Whisky Database), Empfehlungen (cross-link to `/my-taste/recommendations`), Wissen (Lexicon at `/discover/lexicon`, Distilleries at `/discover/distilleries` with integrated List/Map toggle, Tasting Guide at `/guide`, Tasting Templates at `/discover/templates`, Independent Bottlers at `/discover/bottlers`), Über CaskSense (About CaskSense at `/discover/about`, Spende/Hospiz at `/discover/donate`). Live platform stats (taster count, whisky count) shown at top.
-   **Native Dark Warm Detail Pages**: `/my-taste/flavors` (redirects to `/my-taste/profile`), `/my-taste/compare` (RadarChart side-by-side whisky comparison — compares your personal ratings), `/my-taste/analytics` (2 cards: Taste Evolution, Rating Consistency), `/my-taste/benchmark` (standalone Benchmark Analyzer with SimpleShell), `/my-taste/recommendations` (personalized whisky suggestions, moved from Discover), `/my-taste/profile` (unified Flavor Profile: aroma wheel + radar chart + region/cask/peat breakdowns + rating style + top whiskies — single scrollable page), `/my-taste/journal` (3 tabs: Journal, Verkostet, Export), `/my-taste/collection` (Whiskybase collection with sync), `/my-taste/wishlist` (wishlist with AI identification), `/my-taste/settings` (profile settings: name, email, bio, preferences, password, API key), `/my-taste/wheel` (interactive multi-tier flavor wheel from journal/rating data), `/my-taste/pairings` (AI-powered food pairing suggestions per tasting), `/discover/lexicon` (searchable accordion with i18n EN/DE), `/discover/community` (tabbed Twins/Rankings/Leaderboard), `/discover/distilleries` (searchable/filterable encyclopedia with List/Map toggle), `/discover/recommendations` (redirects to `/my-taste/recommendations`), `/discover/database` (searchable whisky database), `/discover/activity` (activity feed timeline), `/discover/templates` (style-specific vocabulary guides for whisky tasting), `/discover/bottlers` (independent bottlers encyclopedia with search and country filter), `/discover/about` (CaskSense story with author photo), `/discover/donate` (Hospiz donation page with PayPal), `/guide` and `/discover/guide` (illustrated tasting assessment guide, dark warm), `/recap/:id` (tasting recap with PDF export).
-   **SimpleLegacyShell**: Routes for legacy features are wrapped in a `SimpleLegacyShell` to provide consistent header and bottom navigation, while preserving legacy content in a white container. Most personal and discover pages have been ported to Dark Warm SimpleShell; legacy routes now redirect to new dark routes.
