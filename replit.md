...
-   **Custom AI Prompts**: All 4 AI generation endpoints accept an optional `customPrompt` parameter (max 500 chars, trimmed/sanitized server-side) that appends user focus instructions to the AI prompt. Frontend UIs: collapsible "Focus Hint" in Connoisseur Report (`M2TasteConnoisseur.tsx`) and Session Story (`M2TastingsHost.tsx`), inline input in Wishlist Summary (`M2TasteWishlist.tsx`) and Whisky Insights (`focused-tasting.tsx`). AI Insights bypasses cache when custom prompt provided. Full i18n EN+DE via `customPrompt.*` keys.
