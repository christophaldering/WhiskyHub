...
-   **CaskSense Labs (`/labs/*`)**: A parallel experimental UX layer focused on refined whisky tasting. Features: Landing page with Join/Host/Solo entry points, Join flow (code-based tasting entry), Tastings overview with status/role filtering, Tasting detail pages, Live Tasting Room (structured rating: Nose/Taste/Finish/Balance/Overall with auto-save), Host Dashboard (create/manage tastings, participant tracking, session control), Results page (rankings, aggregated scores, revealed bottles), Taste page (personal tasting snapshot), Circle page (community connections, activity feed). Dedicated shell (`LabsLayout.tsx`) with warm dark premium theme (`labs-theme.css`), Apple-inspired design, bottom nav (Tastings/Taste/Circle). Reuses all existing backend APIs, auth, and database — no schema changes. Files: `client/src/labs/` directory.
