...
-   **Navigation Structure**: Menu organized into 9 groups: Dashboard (lobby, news, sessions, calendar), My Profile (profile, flavor profile w/ flavor wheel tab, reminders), My Tastings (history, journal, "me as host", recap, export notes), My Whiskys (tasted, wishlist, collection, database), Tools & Analytics (recommendations, comparison, templates, pairings, whisky library, analytics), Community, Whisky Knowledge, About CaskSense, Account (settings, data export, admin). Badges hidden from menu but route preserved.
...
-   **Data Source Tracking**: Journal entries have a `source` field (casksense/imported/whiskybase). "Meine verkosteten Whiskys" and Geschmacksprofil pages include data source filters. CaskSense session data is default; imported/external data only shown when actively enabled by the user.
