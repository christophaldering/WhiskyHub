...
-   **My Taste (Personal Dashboard)**: Personal whisky profile hub requiring email+password sign-in (no naked participants). Shows Taste Snapshot (stability/exploration/smoke scores), AI Taste Insight, and 4 grouped sections: Mein Profil (Flavor Wheel `/my-taste/flavors`, Flavor Profile), Auswertungen (Analytics `/my-taste/analytics` threshold-gated at 10, Comparison `/my-taste/compare`, Benchmark Analyzer), Meine Daten (Journal, My Whiskies, Tasting Recap, Export Notes), Meine Sammlung (Collection/Whiskybase sync, Wishlist).
