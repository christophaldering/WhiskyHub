...
-   **Full Host Dashboard** (`/legacy/tasting/host`): Comprehensive host cockpit with 6 sections: (1) Stat cards (tastings, participants, whiskies), (2) Quick Actions (new tasting, sessions, export, vocabulary) + Next Tasting highlight, (3) Average Scores chart + Documents panel (PDF downloads), (4) Top Whiskies + Tools & Analytics panel, (5) Recent Tastings with per-session action buttons (Results, Preview), (6) Calendar + Invitations info. Desktop-first 2-3 column grid layout, responsive to mobile.
...
-   **My Taste (Personal Dashboard)**: Personal whisky profile hub with Taste Snapshot (stability/exploration/smoke scores), AI Taste Insight, and navigation cards for Flavor Wheel (`/my-taste/flavors`), Comparison (`/my-taste/compare`), Analytics (`/my-taste/analytics`, threshold-gated at 10 whiskies), and Journal. No redundant "Log a Whisky" — that's in the Log tab.
