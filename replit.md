...
-   **Historical Tastings (`/m2/taste/historical`)**: Browsable archive of externally imported tasting data (Excel), independent from CaskSense live tastings. Features expandable tasting cards with whisky rankings, score bars, cross-tasting analytics (top 10, region breakdown, smoky ratio, score distribution), and search. Data stored in `historical_tastings`, `historical_tasting_entries`, `historical_import_runs` tables. Import via `POST /api/admin/historical/import` (admin only, idempotent). Service: `server/historical-import.ts`.
