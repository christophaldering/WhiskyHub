...
export default function LabsVoiceProbe() {
  ...
  const [savedDefault, setSavedDefault] = useState<{ voice: string; mode: string; reserve: string; spark: string; warmth: string; glimmerIntensity: string } | null>(null);
  const [reserve, setReserve] = useState("mid");
  const [spark, setSpark] = useState("mid");
  const [warmth, setWarmth] = useState("mid");
  const [glimmerIntensity, setGlimmerIntensity] = useState("1.5");
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (data?.glimmerIntensity) setGlimmerIntensity(data.glimmerIntensity);
        setSavedDefault({ voice: data.voice, mode: data.mode, reserve: data.reserve, spark: data.spark, warmth: data.warmth, glimmerIntensity: data.glimmerIntensity });
      } catch { /* noop */ }
    })();
  ...
  const saveDefault = async () => {
    ...
    setSaveMsg("");
    try {
      const res = await fetch("/api/admin/cooper-defaults", { method: "POST", headers: { "Content-Type": "application/json", ...pidHeaders() }, body: JSON.stringify({ voice, mode, reserve, spark, warmth, glimmerIntensity }) });
      ...
      {status !== "connected" && (
        <div style={{ display: "flex", flexDirection: "column", gap: SP.md }}>
          {[
            ...
            { key: "glimmerIntensity", label: "Glimmer-Intensität", value: glimmerIntensity, setter: setGlimmerIntensity, opts: [["1", "Dezent"], ["1.5", "Kräftig"], ["2", "Sehr kräftig"], ["2.6", "Maximal"]] },
          ].map((knob) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: SP.sm }}>
          ...
          {savedDefault && (
            <div style={{ fontFamily: FONT.body, fontSize: 12, color: LABS_THEME.faint }}>
              Ton: Zurückhaltung {RES_LABEL[savedDefault.reserve] ?? savedDefault.reserve} · Funken {SPK_LABEL[savedDefault.spark] ?? savedDefault.spark} · Wärme {WRM_LABEL[savedDefault.warmth] ?? savedDefault.warmth} · Glimmer ×{savedDefault.glimmerIntensity}
            </div>
          )}
          <button
