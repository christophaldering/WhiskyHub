      ...
      <div className={`flex flex-col ${reverse ? "md:flex-row-reverse" : "md:flex-row"} gap-8 md:gap-14 items-center`}>
        ...
        <div className="w-full md:w-1/2 space-y-5">
          ...
          <blockquote className="border-l-2 border-primary/40 pl-4 py-1">
            <p className="text-sm md:text-base italic text-primary/80 font-serif">
              {quote}
            </p>
          </blockquote>
        </div>
...
export default function AboutMethod() {
  ...
  const sections = [
    { image: aboutNose, titleKey: "section1Title", bodyKey: "section1Body", quoteKey: "section1Quote" },
    { image: aboutTaste, titleKey: "section2Title", bodyKey: "section2Body", quoteKey: "section2Quote" },
    { image: aboutReflect, titleKey: "section3Title", bodyKey: "section3Body", quoteKey: "section3Quote" },
    { image: aboutJournal, titleKey: "section4Title", bodyKey: "section4Body", quoteKey: "section4Quote" },
  ];
      ...
      >
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6">
          ...
          <motion.p
            ...
            className="text-base md:text-lg text-muted-foreground font-serif italic"
            data-testid="text-about-hero-subtitle"
          >
            {t("aboutMethod.heroSubtitle")}
          </motion.p>
          <motion.div
            initial={{ scaleX: 0 }}
            animate={{ scaleX: 1 }}
            transition={{ delay: 1.1, duration: 0.8 }}
            className="mt-8 h-px w-24 mx-auto bg-primary/30"
          />
        </div>
      </motion.div>
      ...
      <div className="max-w-5xl mx-auto px-6">
        {sections.map((s, i) => (
          <Section
            key={i}
            image={s.image}
            title={t(`aboutMethod.${s.titleKey}`)}
            body={t(`aboutMethod.${s.bodyKey}`)}
            quote={t(`aboutMethod.${s.quoteKey}`)}
            reverse={i % 2 === 1}
            index={i + 1}
          />
        ))}
        ...
        >
          <h2 className="text-2xl md:text-3xl font-serif font-bold text-primary mb-5" data-testid="text-about-closing-title">
            {t("aboutMethod.closingTitle")}
          </h2>
          <p className="text-muted-foreground font-serif leading-relaxed max-w-xl mx-auto text-base md:text-lg mb-10">
            {t("aboutMethod.closingBody")}
          </p>
          <button
            onClick={() => navigate("/")}
