...
async function exportPdf(data: ResultsData) {
  ...
  await saveJsPdf(doc, `${data.title.replace(/[^a-zA-Z0-9]/g, "_")}_results.pdf`);
}
      ...
      {open && (
        ...
        >
          ...
          <button
            ...
            onMouseEnter={e => (e.currentTarget.style.background = `${c.accent}15`)}
            onMouseLeave={e => (e.currentTarget.style.background = "none")}
            onClick={async () => { await exportPdf(data); setOpen(false); }}
