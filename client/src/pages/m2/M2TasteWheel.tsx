...
export default function M2TasteWheel() {
  ...
  const { categoryFreqs, subFreqs, totalMentions, topCategory, mostUniqueFlavor, innerData, outerData } = useMemo(() => {
    ...
    if (combinedEntries.length === 0) {
      return {
        categoryFreqs: {} as Record<string, number>,
        subFreqs: {} as Record<string, Record<string, number>>,
        totalMentions: 0,
        topCategory: null as FlavorCategory | null,
        mostUniqueFlavor: null as { cat: FlavorCategory; sub: { en: string; de: string }; count: number } | null,
        innerData: [] as any[],
        outerData: [] as any[],
      };
    }
    ...
    <div style={{ padding: "16px", maxWidth: 600, margin: "0 auto" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 16, marginTop: 8 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 4 }}>
          <CircleDot style={{ width: 22, height: 22, color: v.accent }} />
          <h1
            style={{
              fontSize: 22,
              fontWeight: 700,
          ...
          >
            {t("m2.wheel.title", "Aroma Wheel")}
          </h1>
        </div>
        <p style={{ fontSize: 13, color: v.muted, margin: 0 }}>
          {t("m2.wheel.subtitle", "Interactive visualization of your sensory history")}
        </p>
      </div>
      ...
      {!pid && (
        ...
        >
          <CircleDot style={{ width: 40, height: 40, color: v.mutedLight, margin: "0 auto 12px", opacity: 0.3 }} />
          <p style={{ fontSize: 14, color: v.muted, margin: 0 }}>
            {t("m2.wheel.signIn", "Sign in to see your aroma wheel.")}
          </p>
        </div>
      )}

      {pid && isLoading && (
        <div style={{ background: v.card, border: `1px solid ${v.border}`, borderRadius: 12, padding: 32, textAlign: "center" }}>
          <p style={{ fontSize: 13, color: v.muted, margin: 0 }}>{t("common.loading", "Loading…")}</p>
        </div>
      )}
      ...
      {pid && !isLoading && (
        <>
          <p
            style={{ fontSize: 12, color: v.mutedLight, textAlign: "center", margin: "0 0 12px" }}
            data-testid="text-wheel-source-count"
          >
            {t("m2.wheel.basedOn", "Based on {{journal}} journal entries and {{notes}} tasting notes", {
              journal: journalEntries?.length || 0,
              notes: ratingNotes?.length || 0,
            })}
          </p>
          ...
          {!hasData ? (
            ...
            >
              <CircleDot style={{ width: 40, height: 40, color: v.mutedLight, margin: "0 auto 12px", opacity: 0.3 }} />
              <p style={{ fontSize: 14, color: v.muted, margin: 0, fontFamily: "'Playfair Display', serif" }}>
                {t("m2.wheel.empty", "No flavor data yet — log some whiskies to build your wheel.")}
              </p>
            </div>
          ) : (
            <>
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                ...
                >
                  <div style={{ fontSize: 22, fontWeight: 700, color: v.accent, fontFamily: "'Playfair Display', serif" }}>
                    {totalMentions}
                  </div>
                  <div style={{ fontSize: 10, color: v.muted, marginTop: 2 }}>
                    {t("m2.wheel.totalMentions", "Total Mentions")}
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  <div style={{ fontSize: 22, fontWeight: 700, color: topCategory?.color || v.accent, fontFamily: "'Playfair Display', serif" }}>
                    {topCategory ? topCategory.en : "—"}
                  </div>
                  <div style={{ fontSize: 10, color: v.muted, marginTop: 2 }}>
                    {t("m2.wheel.topCategory", "Top Category")}
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  <div style={{ fontSize: 22, fontWeight: 700, color: v.accent, fontFamily: "'Playfair Display', serif", opacity: 0.8 }}>
                    {mostUniqueFlavor ? mostUniqueFlavor.sub.en : "—"}
                  </div>
                  <div style={{ fontSize: 10, color: v.muted, marginTop: 2 }}>
                    {t("m2.wheel.mostUnique", "Most Unique")}
                  </div>
                </div>
              </div>
              ...
              >
                <h2 style={{ fontSize: 15, fontWeight: 600, color: v.text, margin: "0 0 2px", fontFamily: "'Playfair Display', serif" }}>
                  {t("m2.wheel.yourWheel", "Your Aroma Wheel")}
                </h2>
                <p style={{ fontSize: 11, color: v.muted, margin: "0 0 4px" }}>
                  {t("m2.wheel.tapHint", "Tap a segment to explore subcategories")}
                </p>
                      ...
                      <Tooltip
                        content={({ payload }: any) => {
                          ...
                          return (
                            ...
                            >
                              <div style={{ fontSize: 13, fontWeight: 600, color: v.text, fontFamily: "'Playfair Display', serif" }}>
                                {data.name}
                              </div>
                              <div style={{ fontSize: 11, color: v.muted }}>
                                {data.actualValue} {t("m2.wheel.mentions", "mentions")}
                              </div>
                            </div>
                          );
                        }}
                  ...
                  {FLAVOR_WHEEL_DATA.map((cat) => (
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 500,
                        cursor: "pointer",
                        border: selectedCategory === cat.id ? `1px solid ${v.accent}` : `1px solid ${v.border}`,
                        background: selectedCategory === cat.id ? v.elevated : v.card,
                        color: v.text,
                    ...
                    >
                      ...
                      <span>{cat.en}</span>
                      <span style={{ color: v.muted }}>({categoryFreqs[cat.id] || 0})</span>
                    </button>
                  ))}
                </div>
              </div>
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ fontSize: 16, fontWeight: 600, color: v.text, fontFamily: "'Playfair Display', serif" }}>
                        {selectedCatData.en}
                      </span>
                      <span style={{ fontSize: 12, color: v.muted }}>
                        ({categoryFreqs[selectedCatData.id]} {t("m2.wheel.mentions", "mentions")})
                      </span>
                    </div>
                    <button
                      onClick={() => setSelectedCategory(null)}
                      style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: v.muted }}
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <span style={{ fontSize: 13, fontWeight: 500, color: v.text }}>{sub.en}</span>
                            <span
                              style={{
                                fontSize: 13,
                                fontWeight: 700,
                                fontFamily: "'Playfair Display', serif",
                                color: count > 0 ? selectedCatData.color : v.mutedLight,
                              }}
                            >
                              {count}
                            </span>
