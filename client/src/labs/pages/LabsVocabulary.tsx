...
import { Copy, Check, ChevronDown, ChevronUp, Lightbulb, MessageSquare, ChevronLeft } from "lucide-react";
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ fontSize: 10, fontWeight: 600, color: "var(--labs-accent)", textTransform: "uppercase", letterSpacing: 1 }}>{title}</span>
        <button onClick={() => onCopy(terms.join(", "), id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 3, color: isCopied ? "var(--labs-success)" : "var(--labs-text-muted)" }} data-testid={`button-vcopy-${id}`}>
...
export default function LabsVocabulary() {
  ...
  return (
    <div className="px-5 py-6 max-w-2xl mx-auto" data-testid="labs-discover-vocabulary-page">
      <Link href="/labs/discover" style={{ textDecoration: "none" }}>
        <button className="labs-btn-ghost mb-4" style={{ display: "flex", alignItems: "center", gap: 4 }} data-testid="button-back-vocabulary">
          <ChevronLeft className="w-4 h-4" /> Back
        </button>
      </Link>

      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
        <MessageSquare style={{ width: 22, height: 22, color: "var(--labs-accent)" }} />
        <h1 className="labs-serif" style={{ fontSize: 22, fontWeight: 700, color: "var(--labs-text)", margin: 0 }} data-testid="text-vocabulary-title">
          Vocabulary
        </h1>
      </div>
      <p style={{ fontSize: 13, color: "var(--labs-text-muted)", margin: "0 0 20px" }}>Copy-paste vocabulary cards for your tasting notes</p>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {CATEGORY_IDS.map((id) => {
          ...
          return (
            <div key={id} className="labs-card" style={{ overflow: "hidden" }} data-testid={`labs-vocab-${id}`}>
              <button onClick={() => setExpandedId(isOpen ? null : id)} className="labs-btn-ghost" style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", textAlign: "left" }} data-testid={`button-toggle-vocab-${id}`}>
              ...
              {isOpen && (
                <div style={{ padding: "0 16px 16px", borderTop: "1px solid var(--labs-border)" }}>
                  ...
                  <div style={{ marginTop: 16, padding: "10px 12px", background: "var(--labs-surface-elevated)", borderRadius: 8, border: "1px solid var(--labs-border)", display: "flex", gap: 8, alignItems: "flex-start" }}>
                    <Lightbulb style={{ width: 12, height: 12, color: "var(--labs-accent)", marginTop: 2, flexShrink: 0 }} />
                    <div>
                      <div style={{ fontSize: 10, fontWeight: 600, color: "var(--labs-accent)", marginBottom: 3 }}>Expert Tip</div>
