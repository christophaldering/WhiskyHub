...
async function exportStoryPdf(storyData: any) {
  const { tasting, sortedRanking, participants, eventPhotos, winner, winnerNarration, aiComments } = storyData;
  ...
  const drawHeader = () => { doc.setFillColor(...accent); doc.rect(0, 0, pageW, 2.5, "F"); };
  const drawFooter = (pageLabel?: string) => {
    const y = pageH - 11;
    doc.setDrawColor(...accent); doc.setLineWidth(0.25); doc.line(marginX, y - 3, pageW - marginX, y - 3);
    doc.setFontSize(6.5); doc.setTextColor(...muted); doc.setFont("helvetica", "normal");
    doc.text("CaskSense Story", marginX, y);
    doc.text(pageLabel || tasting.title || "Tasting", pageW - marginX, y, { align: "right" });
  };
  ...
  for (const w of (sortedRanking || []).slice(0, 10)) {
    if (w.coverImageUrl || w.imageUrl) {
      const url = w.coverImageUrl || w.imageUrl;
      whiskyImgB64s.set(w.id, await fetchImageAsBase64(url));
    }
  }
  ...
  // Cover event photo (first one, placed at top with text below)
  const firstPhoto = eventPhotoB64s[0];
  if (firstPhoto) {
    try {
      doc.addImage(firstPhoto, "JPEG", 0, 0, pageW, 100, undefined, "FAST");
      y = 114;
    } catch { /* skip on error */ }
  }

  doc.setFontSize(8); doc.setTextColor(...accent); doc.setFont("helvetica", "bold");
  doc.text("CASKSENSE STORY", pageW / 2, y, { align: "center" }); y += 10;
  doc.setFontSize(22); doc.setTextColor(...textColor); doc.setFont("helvetica", "bold");
  const titleLines = doc.splitTextToSize(tasting.title || "Tasting Story", contentW);
  titleLines.forEach((l: string) => { doc.text(l, pageW / 2, y, { align: "center" }); y += 10; });
  y += 2;
  doc.setFontSize(9); doc.setTextColor(...muted); doc.setFont("helvetica", "normal");
  doc.text(`${tasting.date || ""}  ·  ${tasting.location || ""}`, pageW / 2, y, { align: "center" });
  y = y + 12;

  doc.setDrawColor(...accent); doc.setLineWidth(0.4); doc.line(marginX, y, pageW - marginX, y); y += 12;
  ...
  if (tasters.length > 0) {
    ...
    doc.text(`${tasters.length} VERKOSTER`, pageW / 2, y, { align: "center" }); y += 6;
    ...
    nlines.forEach((line: string) => { doc.text(line, pageW / 2, y, { align: "center" }); y += 5.5; });
    y += 8;
  }

  // Quick ranking overview on cover
  doc.setFontSize(7.5); doc.setTextColor(...accent); doc.setFont("helvetica", "bold");
  doc.text("RANKING", marginX, y); y += 6;
  (sortedRanking || []).slice(0, 5).forEach((w: any, i: number) => {
    doc.setFont("helvetica", i === 0 ? "bold" : "normal");
    doc.setFontSize(9); doc.setTextColor(...(i < 3 ? accent : textColor));
    const medal = i === 0 ? "#1" : i === 1 ? "#2" : i === 2 ? "#3" : `#${i + 1}`;
    doc.text(`${medal}  ${(w.name || "?").slice(0, 42)}`, marginX, y);
  ...
  // ===== PAGES: PER-WHISKY ACT PAGES =====
  for (let i = 0; i < (sortedRanking || []).length; i++) {
    const w = sortedRanking[i];
    drawFooter(); doc.addPage(); drawBg(); drawHeader();
    y = 22;

    // Act label
    doc.setFontSize(7); doc.setTextColor(...muted); doc.setFont("helvetica", "bold");
    doc.text(`ACT II · WHISKY ${i + 1} / ${sortedRanking.length}`, marginX, y); y += 8;

    // Rank badge
    const rankText = `#${i + 1}`;
    doc.setFontSize(28); doc.setTextColor(...(i < 3 ? accent : muted)); doc.setFont("helvetica", "bold");
    doc.text(rankText, marginX, y); y += 2;

    // Try whisky photo on right
    const imgB64 = whiskyImgB64s.get(w.id);
    const hasImg = !!imgB64;
    if (hasImg) {
      try {
        doc.addImage(imgB64!, "JPEG", pageW - marginX - 55, 18, 55, 70, undefined, "FAST");
      } catch { /* skip */ }
    }

    // Name + distillery
    const nameMaxW = hasImg ? contentW - 62 : contentW;
    doc.setFontSize(16); doc.setTextColor(...textColor); doc.setFont("helvetica", "bold");
    const wNameLines = doc.splitTextToSize(w.name || "Unknown", nameMaxW);
    wNameLines.slice(0, 2).forEach((l: string) => { doc.text(l, marginX, y); y += 8; });
    if (w.distillery) {
      doc.setFontSize(9); doc.setTextColor(...muted); doc.setFont("helvetica", "normal");
      doc.text(w.distillery + (w.region ? ` · ${w.region}` : ""), marginX, y); y += 7;
    }
    if (w.abv) {
      doc.setFontSize(8.5); doc.setTextColor(...muted);
      doc.text(`${w.abv}% ABV`, marginX, y); y += 6;
    }
    y = Math.max(y, hasImg ? 96 : y) + 4;

    // Score bar
    if (w.avgOverall != null) {
      doc.setFillColor(...accent); doc.rect(marginX, y - 4, 3, 16, "F");
      doc.setFontSize(11); doc.setTextColor(...accent); doc.setFont("helvetica", "bold");
      doc.text(formatScore(w.avgOverall), marginX + 7, y + 4);
      doc.setFontSize(7.5); doc.setTextColor(...muted); doc.setFont("helvetica", "normal");
      doc.text("Ø SCORE", marginX + 7, y + 10); y += 22;
    }
    ...
    if (validScores.length > 0) {
      ...
      validScores.forEach(([label, val], si) => {
        const cx = marginX + si * colW;
        doc.setFontSize(8); doc.setTextColor(...accent); doc.setFont("helvetica", "bold");
        doc.text(formatScore(val!), cx, y);
        doc.setFontSize(6.5); doc.setTextColor(...muted); doc.setFont("helvetica", "normal");
    ...
    doc.setDrawColor(...muted); doc.setLineWidth(0.2); doc.line(marginX, y, pageW - marginX, y); y += 8;

    // AI comment
    if (aiComments?.[w.name]) {
      doc.setFontSize(8); doc.setTextColor(...accent); doc.setFont("helvetica", "bold");
      doc.text("STORY", marginX, y); y += 5;
      y = addTextWrapped(`"${aiComments[w.name]}"`, marginX, y, contentW, 8.5, muted, "italic") + 5;
    }

    // Taster ratings excerpts
    if (w.ratings && w.ratings.length > 0) {
      ...
      if (notesArr.length > 0) {
        ...
        for (const r of notesArr) {
          if (y > pageH - 30) break;
          const name = tasters.find((t: any) => t.participantId === r.participantId)?.participant?.name || "";
          doc.setFontSize(7); doc.setTextColor(...muted); doc.setFont("helvetica", "bold");
          if (name) doc.text(stripGuestSuffix(name).toUpperCase(), marginX, y);
          y += 4;
          y = addTextWrapped(r.notes.slice(0, 180), marginX + 3, y, contentW - 3, 7.5, textColor) + 3;
        }
      }
    }
  }
  ...
  if (photoEntries.length > 0) {
    ...
    doc.text("EVENT FOTOS", pageW / 2, y, { align: "center" }); y += 8;
    ...
    for (let pi = 0; pi < photoEntries.length; pi++) {
      ...
      if (y + photoH > pageH - 22) {
        drawFooter("Event Fotos"); doc.addPage(); drawBg(); drawHeader(); y = 20;
        doc.setFontSize(8.5); doc.setTextColor(...accent); doc.setFont("helvetica", "bold");
        doc.text("EVENT FOTOS", pageW / 2, y, { align: "center" }); y += 8;
        col = 0;
      }
      try {
        ...
        if (caption) {
          ...
          doc.text(caption.slice(0, 38), px, y + photoH + 3.5);
        }
      } catch { /* skip broken image */ }
      col++;
      if (col >= cols) { col = 0; y += photoH + 12; }
    }
  ...
  if (winner) {
    ...
    // Try to show winner whisky photo large
    const winnerImg = whiskyImgB64s.get(winner.id);
    if (winnerImg) {
      try {
        doc.addImage(winnerImg, "JPEG", (pageW - 80) / 2, y, 80, 80, undefined, "FAST");
        y += 88;
      } catch { /* skip */ }
    }

    doc.setFontSize(9); doc.setTextColor(...accent); doc.setFont("helvetica", "bold");
    doc.text("DER SIEGER DES ABENDS", pageW / 2, y, { align: "center" }); y += 10;
    doc.setFontSize(22); doc.setTextColor(...textColor); doc.setFont("helvetica", "bold");
    const winLines = doc.splitTextToSize(winner.name || "", contentW);
    winLines.slice(0, 2).forEach((l: string) => { doc.text(l, pageW / 2, y, { align: "center" }); y += 10; });
    if (winner.distillery) {
      doc.setFontSize(9.5); doc.setTextColor(...muted); doc.setFont("helvetica", "normal");
      doc.text(winner.distillery, pageW / 2, y, { align: "center" }); y += 6;
    }
    if (winner.avgOverall != null) {
      doc.setFontSize(18); doc.setTextColor(...accent); doc.setFont("helvetica", "bold");
      doc.text(formatScore(winner.avgOverall), pageW / 2, y + 8, { align: "center" }); y += 18;
    }
    if (winnerNarration) {
      y += 6;
      doc.setDrawColor(...accent); doc.setLineWidth(0.3); doc.line(marginX + 20, y, pageW - marginX - 20, y); y += 10;
      y = addTextWrapped(`"${winnerNarration}"`, marginX + 8, y, contentW - 16, 9.5, muted, "italic") + 6;
    }
    drawFooter("Sieger");
  }

  // Rankings full page (all whiskies with AI notes)
  doc.addPage(); drawBg(); drawHeader();
  y = 22;
  doc.setFontSize(8.5); doc.setTextColor(...accent); doc.setFont("helvetica", "bold");
  doc.text("VOLLSTÄNDIGES RANKING", marginX, y); y += 8;
  for (let i = 0; i < (sortedRanking || []).length; i++) {
    const w = sortedRanking[i];
    if (y > pageH - 30) { drawFooter("Ranking"); doc.addPage(); drawBg(); drawHeader(); y = 22; }
    doc.setFont("helvetica", i === 0 ? "bold" : "normal");
    doc.setFontSize(9); doc.setTextColor(...(i < 3 ? accent : textColor));
    doc.text(`#${i + 1}  ${(w.name || "?").slice(0, 44)}`, marginX, y);
    if (w.avgOverall != null) {
      doc.setFont("helvetica", "bold");
      doc.text(formatScore(w.avgOverall), pageW - marginX, y, { align: "right" });
    }
    ...
    if (aiComments?.[w.name]) {
      ...
      const clines = doc.splitTextToSize(aiComments[w.name], contentW - 8);
      clines.slice(0, 2).forEach((cl: string) => { doc.text(cl, marginX + 6, y); y += 4; });
    }
    y += 3;
  }
  drawFooter("Ranking");
