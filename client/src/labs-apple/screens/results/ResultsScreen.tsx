...
const MarkdownText: React.FC<{ text: string; th: ThemeTokens }> = ({ text, th }) => {
  ...
  return (
    <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 17, lineHeight: 1.7, color: th.text }}>
      {lines.map((line, i) => {
        if (line.startsWith('## ')) return <h3 key={i} style={{ fontFamily: 'Playfair Display, serif', fontSize: 20, fontWeight: 600, margin: '20px 0 8px' }}>{line.slice(3)}</h3>
...
// ── InsightsTab ─────────────────────────────────────────────────────────────
const InsightsTab: React.FC<{ th: ThemeTokens; t: Translations; tastingId: string; participantId: string }> = ({ th, t, tastingId, participantId }) => {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: SP.xs, padding: `${SP.sm}px ${SP.md}px`, overflowX: 'auto' }}>
        {tabs.map((tab, i) => (
          <button key={i} onClick={() => setActive(i)} style={{ flexShrink: 0, height: 36, padding: '0 14px', borderRadius: 18, border: 'none', cursor: 'pointer', background: activeTab === i ? th.gold : th.bgCard, color: activeTab === i ? '#1a0f00' : th.muted, fontSize: 13, fontWeight: activeTab === i ? 700 : 400, transition: 'all 150ms' }}>{tab}</button>
        ))}
      </div>
      ...
      <div style={{ padding: SP.md }}>
        {activeTab === 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: SP.sm }}>
            {[
              { icon: <Icon.Star color={th.gold} size={20} />, label: t.resultsFavorite, value: myRatings.length > 0 ? whiskies.find(w => w.id === myRatings.sort((a, b) => b.score - a.score)[0]?.whiskeyId)?.name || '—' : '—' },
              { icon: <Icon.Trophy color={th.gold} size={20} />, label: t.resultsGroupWinner, value: allOverall > 0 ? `Ø ${Math.round(allOverall)}` : '—' },
        ...
        {activeTab === 2 && (
          <div>
            ...
            {whiskies.map((w, i) => {
              const wRatings = ratings.filter(r => r.whiskeyId === w.id && r.dimension === 'overall')
              const wAvg = Math.round(avg(wRatings.map(r => r.score)))
              const pct = ((wAvg - 60) / 40) * 100
              return (
        ...
        {activeTab === 3 && (
          <div>
            ...
            {(['nose', 'palate', 'finish', 'overall'] as const).map(dim => {
              const dimRatings = myRatings.filter(r => r.dimension === dim)
              const dimAvg = Math.round(avg(dimRatings.map(r => r.score)))
              const pct = ((dimAvg - 60) / 40) * 100
...
const ConnoisseurReport: React.FC<{ th: ThemeTokens; t: Translations; participantId: string }> = ({ th, t, participantId }) => {
  ...
  const generate = async () => {
    setGen(true)
    try {
      const res = await fetch(`/api/participants/${participantId}/connoisseur-report`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-participant-id': participantId }, body: JSON.stringify({ language: lang }) })
  ...
  if (!active) return (
    <div style={{ padding: SP.lg, textAlign: 'center' }}>
      ...
      <button onClick={generate} disabled={generating} style={{ height: 52, padding: '0 28px', borderRadius: 16, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg, ${th.gold}, ${th.amber})`, color: '#1a0f00', fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, margin: '0 auto' }}>
    ...
    <div style={{ padding: SP.md }}>
      <div style={{ display: 'flex', gap: SP.xs, marginBottom: SP.md, overflowX: 'auto' }}>
        {reportTabs.map((tab, i) => (
          <button key={i} onClick={() => setTab(i)} style={{ flexShrink: 0, height: 36, padding: '0 14px', borderRadius: 18, border: 'none', cursor: 'pointer', background: activeTab === i ? th.gold : th.bgCard, color: activeTab === i ? '#1a0f00' : th.muted, fontSize: 13, fontWeight: activeTab === i ? 700 : 400 }}>{tab}</button>
        ))}
      </div>
      {activeTab === 0 && active.content && <MarkdownText text={active.content} th={th} />}
      {activeTab === 0 && !active.content && <div style={{ color: th.muted, fontStyle: 'italic' }}>—</div>}
      ...
      <div style={{ display: 'flex', gap: SP.sm, marginTop: SP.lg }}>
        <button onClick={generate} disabled={generating} style={{ height: 44, padding: '0 16px', borderRadius: 12, border: `1px solid ${th.gold}44`, background: 'none', color: th.gold, cursor: 'pointer', fontSize: 13 }}>{t.connoisseurRegen}</button>
      </div>
    </div>
  )
}
...
// ── ResultsScreen ────────────────────────────────────────────────────────────
interface Props { th: ThemeTokens; t: Translations; tastingId: string; participantId: string; isHost: boolean }

export const ResultsScreen: React.FC<Props> = ({ th, t, tastingId, participantId, isHost }) => {
  const [activeTab, setTab] = useState(0)
  ...
  return (
    <div style={{ minHeight: '100%', background: th.bg, color: th.text, fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: SP.xs, padding: `0 ${SP.md}px ${SP.md}px`, overflowX: 'auto' }}>
        {tabs.map((tab, i) => (
          <button key={i} onClick={() => setTab(i)} style={{ flexShrink: 0, height: 40, padding: '0 16px', borderRadius: 20, border: 'none', cursor: 'pointer', background: activeTab === i ? th.gold : th.bgCard, color: activeTab === i ? '#1a0f00' : th.muted, fontSize: 14, fontWeight: activeTab === i ? 700 : 400, transition: 'all 150ms' }}>{tab}</button>
        ))}
      </div>

      {activeTab === 0 && <InsightsTab th={th} t={t} tastingId={tastingId} participantId={participantId} />}
      {activeTab === 1 && <div style={{ padding: SP.md, color: th.muted, fontStyle: 'italic', fontFamily: 'Cormorant Garamond, serif', fontSize: 16 }}>{t.recapTitle} — Recap wird hier angezeigt.</div>}
      {activeTab === 2 && <ConnoisseurReport th={th} t={t} participantId={participantId} />}
      {activeTab === 3 && isHost && <div style={{ padding: SP.md, color: th.muted, fontStyle: 'italic', fontFamily: 'Cormorant Garamond, serif', fontSize: 16 }}>{t.narrativeTitle}</div>}
    </div>
  )
}
